...
class XiMLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.0):
        ...
        self.widths = [int(self.hidden_features * r) for r in [0.125, 0.25, 0.5, 1.0]]
        self.out_features = out_features or in_features

        self.fc1_conv = nn.Conv2d(in_features, self.hidden_features, 1)
        self.fc1_bn   = nn.BatchNorm2d(self.hidden_features)
        self.fc1_lif  = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')

        self.fc2_conv = nn.Conv2d(self.hidden_features, self.out_features, 1)
        self.fc2_bn   = nn.BatchNorm2d(self.out_features)
        self.fc2_lif  = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')

        for g_id, g in enumerate(self.widths):
            p = 0
            p += g * in_features + (0 if self.fc1_conv.bias is None else g)
            p += 4 * g
            p += g * self.out_features + self.out_features
            print(f"granularity {g_id}: filters: {g} -- {p} params")

    def forward(self, x, granularity=None):
        T,B,C,H,W = x.shape
        if granularity is not None:
            if not isinstance(granularity, int):
                raise TypeError(f"granularity must be an int or None, got {type(granularity)}")
            if not (0 <= granularity < len(self.widths)):
                raise ValueError(f"granularity must be between 0 and {len(self.widths)-1}, got {granularity}")
            
        g = self.widths[granularity] if granularity is not None else random.choice(self.widths)
        g = min(g, self.hidden_features)

        w1 = self.fc1_conv.weight[:g]
        b1 = None if self.fc1_conv.bias is None else self.fc1_conv.bias[:g]
        bn = self.fc1_bn
        x1 = nn.functional.conv2d(x.flatten(0,1), w1, b1)
        x1 = nn.functional.batch_norm(x1,
            bn.running_mean[:g], bn.running_var[:g],
            bn.weight[:g], bn.bias[:g],
            self.training, bn.eps
        )
        x1 = self.fc1_lif(x1.reshape(T,B,g,H,W))

        w2 = self.fc2_conv.weight[:, :g]
        x2 = nn.functional.conv2d(x1.flatten(0,1), w2, self.fc2_conv.bias)
        x2 = self.fc2_bn(x2).reshape(T,B,self.out_features,H,W)
        return self.fc2_lif(x2)

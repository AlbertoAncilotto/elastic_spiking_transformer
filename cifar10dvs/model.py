...
class XiSSA(nn.Module):
    ...
    def _generate_head_granularities(self, max_heads, num_granularities):
        if num_granularities == 1:
            return [max_heads]
        min_heads = max(4, max_heads // (2 ** (num_granularities - 1)))
        granularities = np.logspace(
            np.log2(min_heads), 
            np.log2(max_heads), 
            num_granularities, 
...
class XiConvMultiGran(nn.Module):
    ...
    def _generate_granularities(self, max_channels, num_granularities):
        ...
        min_channels = max(8, max_channels // (2 ** (num_granularities-1)))
        granularities = np.logspace(
...
class XiSPSv2(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256, alpha=1.0, num_granularities=1):
        ...
        if num_granularities>1:
            self.proj_conv3 = XiConvMultiGran(c3, embed_dims, kernel_size=3, stride=1, padding=1, num_granularities=num_granularities)
        else:
        ...
        if num_granularities>1:
            self.rpe_conv = XiConvMultiGran(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1, num_granularities=num_granularities)
        else:

...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    # Maintain 4 accuracy meters (top1 & top5)
    top1_all = [AverageMeter() for _ in range(4)]
    top5_all = [AverageMeter() for _ in range(4)]
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            for g in range(4): #BOOP
                with amp_autocast():
                    output = model(input, granularity=g)
                ...
                # Distributed reduce
                if args.distributed:
                    loss_val = reduce_tensor(loss.data, args.world_size)
                ...
                if g == 0:  # loss logged only once
                    losses_m.update(loss_val.item(), input.size(0))

                # update accuracy meters
                top1_all[g].update(acc1.item(), output.size(0))
                top5_all[g].update(acc5.item(), output.size(0))
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                for g in range(4):
                    _logger.info(
                        f"Test{log_suffix} [G={g}] [{batch_idx:>4d}/{last_idx}]  "
                        f"Acc@1: {top1_all[g].val:7.4f} ({top1_all[g].avg:7.4f})  "
                        f"Acc@5: {top5_all[g].val:7.4f} ({top5_all[g].avg:7.4f})"
                    )

    # Final output dictionary
    metrics = OrderedDict({
        f'top1_g{g}': top1_all[g].avg for g in range(4)
    })
    metrics.update({
        f'top5_g{g}': top5_all[g].avg for g in range(4)
    })
    metrics['loss'] = losses_m.avg
    metrics['top1'] = top1_all[-1].avg  # for compatibility
